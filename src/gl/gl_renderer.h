...
// Exposed in the header so platform-specific code (main.c) can access FBO
// fields for screenshots.
typedef struct {
  ...
  GLuint whiteTexture; // 1x1 white pixel for drawing primitives (rectangles,
                       // lines, etc.)
  ...
  GLuint hostFramebuffer; // present target for the composited frame, where 0 ==
                          // the window
