...
static RValue builtin_window_enable_borderless_fullscreen(VMContext* ctx, RValue* args, int32_t argCount) {
    if (1 > argCount) return RValue_makeUndefined();
    Runner* runner = ctx->runner;
    if (runner == nullptr) return RValue_makeUndefined();
    bool enable = RValue_toBool(args[0]);
    if (runner->enableBorderlessFullscreen != nullptr) {
        runner->enableBorderlessFullscreen(enable);
    }
    return RValue_makeUndefined();
}
