...
typedef struct RunnerKeyboardState {
  ...
  bool rawKeyDown[GML_KEY_COUNT];
  bool rawKeyPressed[GML_KEY_COUNT];
  bool rawKeyReleased[GML_KEY_COUNT];
  int32_t keyMap[GML_KEY_COUNT]; // keyboard_set_map: incoming key -> reported
                                 // key (identity by default)
...
// Called by platform layer when a key is pressed/released (gmlKeyCode = GML vk_
// code)
...
void RunnerKeyboard_onCharacter(RunnerKeyboardState *kb,
...
bool RunnerKeyboard_checkDirectPressed(RunnerKeyboardState *kb,
                                       int32_t gmlKeyCode);
bool RunnerKeyboard_checkDirectReleased(RunnerKeyboardState *kb,
                                        int32_t gmlKeyCode);

// Simulated press/release (used by keyboard_key_press/keyboard_key_release GML
// functions)
void RunnerKeyboard_simulatePress(RunnerKeyboardState *kb, int32_t gmlKeyCode);
void RunnerKeyboard_simulateRelease(RunnerKeyboardState *kb,
                                    int32_t gmlKeyCode);
...
void RunnerKeyboard_setMap(RunnerKeyboardState *kb, int32_t fromKey,
                           int32_t toKey);
